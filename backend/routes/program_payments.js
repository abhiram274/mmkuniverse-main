...
router.get("/:id/check-guest/:email", async (req, res) => {
  ...
  try {
    const [pending] = await db.query(
      "SELECT id FROM program_payment_requests WHERE program_id = ? AND guest_email = ? AND status = 'pending'",
      [program_id, guest_email]
    );
    ...
    const [joined] = await db.query(
      "SELECT id FROM program_attendees WHERE program_id = ? AND guest_email = ?",
      [program_id, guest_email]
    );

    if (joined.length > 0) {
      return res.json({ status: "You have already enrolled" });
    }
